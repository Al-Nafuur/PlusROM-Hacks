...
;
; PlusROM functions for HSC backend added 
;  by Wolfgang Stubig (Sep 20, 2021)
;
...
ORIGINAL_ROM            = FALSE
   ...
   IF (ORIGINAL_ROM = TRUE && PLUSROM = TRUE)

      echo ""
      echo "*** ERROR: ORIGINAL_ROM and PLUSROM cannot both be set"
      echo ""
      err
...
HIGHSCORE_ID            = 44        ;Smurf game ID in Highscore DB
   ENDIF
...
CheckToAlternatePlayers
  ...
  IF PLUSROM
   ...
   jsr SetPlusRomStateAndForwardToSetGameAudio
...
SetPlusRomStateAndForwardToSetGameAudio
   inc plusRomState
   jmp SetGameAudioValues
...
;--------------------------------------
   ...
   IF PLUSROM
; check for justGameOver
   ldy plusRomState                    ; get current game state
   beq .noSendScores
...
SendPlusROMScore:
   lda #0
   sta plusRomState                 ; dec plusRomState should do the trick too..

   lda reservedPlayerSkillLevel
   sta WriteToBuffer
   lda currentPlayerScore
   sta WriteToBuffer
   lda currentPlayerScore + 1
   sta WriteToBuffer
   lda currentPlayerScore + 2
   sta WriteToBuffer
...
JumpToCurrentGameStateRoutine_BANK1
  ...
  IF PLUSROM
   ...
   .word (PlusROM_API-$D000)        ; PlusRom API pointer
